...
"cliente;" + // Punto y coma significa nueva línea
"detalles;" +
"cantidad, porcentajeIVA, iva, importeTotal;estadoDeLaFactura"
/*"porcentajeCliente, montoCliente;"+
"porcentajeEmpresa, montoEmpresa;"*/
)
//@Tab(baseCondition = "${estadoDeLaFactura} = 1") // Tab sin nombre, es el de por defecto
@Tab(name="Pendiente", baseCondition = "${estadoDeLaFactura} = 1") // Tab con nombre
@Tab(name="Aprobada", baseCondition = "${estadoDeLaFactura} = 2") // Tab con nombre
public class Factura extends Incrementable4 {
    ...
    @ElementCollection
    @ListProperties(
           "cargaDeInventario.codigoDelProducto, cargaDeInventario.producto, cantidad, precioPorUnidad, "  +
        "importe+[" + 
    ...
    int estadoDeLaFactura; // Una referencia Java convencional
